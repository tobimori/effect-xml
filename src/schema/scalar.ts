...
import { isXml10Char } from "../parser/character.ts";

/** Validates typed scalar output before its field path is lost at document serialization. */
    ...
    if (codePoint === undefined || !isXml10Char(codePoint)) {
      ...
      return Effect.fail(
        new SchemaIssue.InvalidValue(
          {
            message: `${context} contains an invalid XML 1.0 character ${label} at UTF-16 offset ${offset}`,
          },
          value,
          options,
        ),
